...
| `thumbnail_widths` | array | no | `[400, 800, 120]` | Pixel widths generated by the image pipeline. Changing this requires regenerating existing variants. |
...
- Original extensions: `.jpg`, `.jpeg`, `.png`, `.webp`, `.gif`. All re-encoded to JPEG on save (plus WebP variants).

### Variant suffix convention

The image pipeline generates three size variants of every uploaded image, in both JPEG and WebP. Variants are stored alongside the original with suffix naming:

| Variant | Suffix | Use |
|---------|--------|-----|
| Original | `<basename>.jpg` | Kept untouched, source for re-thumbnailing |
| Card thumbnail (400px wide) | `<basename>-thumb-400.jpg` + `.webp` | Category page product cards |
| Hero / banner (800px wide) | `<basename>-hero-800.jpg` + `.webp` | Product main image, category banner |
| Gallery thumbnail (120px wide) | `<basename>-thumb-120.jpg` + `.webp` | Gallery thumbnail strip on product page |

Heights scale to maintain aspect ratio — only width is constrained. The `<picture>` element in the template offers the WebP source first with the JPEG as fallback.

The original is preserved so variant widths or quality can be changed later in `config.json` and existing images re-thumbnailed in place.

### JSON references

Image paths in product and category JSON are stored **without** the extension and **without** the variant suffix. The renderer appends both. Example: `"file": "details/angle-1"` resolves to `/shop/media/product-images/sku-001/details/angle-1-hero-800.webp` (or `.jpg`) at render time.

This keeps the JSON variant-agnostic — if the project ever adds a 1600px retina variant, the JSON doesn't change.
...
| `<meta property="og:image">` | URL to primary image at hero-800 variant |
| `<meta property="og:url">` | canonical URL |
| `<meta property="og:type">` | `"product"` |
| `<meta name="twitter:card">` | `"summary_large_image"` |
| `<meta name="twitter:site">` | `seo.twitter_handle` if set |
| JSON-LD `Product` schema | name, description, image array (all variants of primary), brand, sku, url, offers (price, priceCurrency, availability="https://schema.org/InStock", url) |
...
| `<meta property="og:title">` | same as `<title>` |
| `<meta property="og:description">` | same as meta description |
| `<meta property="og:image">` | URL to category banner at hero-800 variant if `image` set, else `seo.og_image` |
| `<meta property="og:url">` | canonical URL |
